"""
CATEGORY: INTERNAL_RUNTIME (HYBRID)
AUTHORITY_LAYER: Runtime Behavioral Truth (Hybrid: External Observable + Runtime Behavioral)
VALIDATES:
  - External contract (HARNESS_CONTRACT coverage via execute_from_input)
  - Internal execution behavior (INTERNAL_RUNTIME coverage via run_workflow)
  - Workflow execution behavior
  - Agent registration
  - Contract execution
ENTRYPOINT: execute_from_input + run_workflow (Hybrid validation)
DIRECT_INTERNAL_CALLS:
  - agent_registry internals
  - orchestrator_runtime internals
MONKEYPATCH_USAGE: NONE
MOCKING_POLICY: REAL_EXECUTION
TEST_INTENT: HYBRID_VALIDATION (Comprehensive validation across external and internal layers)
ARCHITECTURAL_SCOPE: Workflow execution (hybrid external + internal)

---

HYBRID VALIDATION DOCUMENTATION:

This file is a legitimate hybrid validation file that validates both:

1. HARNESS_CONTRACT (External Observable Truth):
   - External contract tests (execute_from_input) — Validates layered contract structure
   - Contract enforcement (status, result/reason fields)
   - External API correctness

2. INTERNAL_RUNTIME (Runtime Behavioral Truth):
   - Internal validation tests (run_workflow) — Validates internal execution behavior
   - Retry behavior validation
   - System entry execution validation
   - Internal orchestrator behavior

Hybrid validation is intentional and appropriate for comprehensive workflow validation.
No structural movement or reclassification required — this is documented as hybrid.
"""
