"""
BACKGROUND EXECUTION MANAGER — Non-Blocking Workflow Wrapper (Phase 2B)
...
- Workflow-level state (QUEUED/ACTIVE/COMPLETED/FAILED) is separate from step-level state
...
class BackgroundManager:
    ...
    def _run_workflow(
        ...
        State transitions:
        - QUEUED → ACTIVE (thread starts execution)
        - ACTIVE → COMPLETED (workflow_fn returns successfully)
        - ACTIVE → FAILED (workflow_fn raises exception)
        """
        with self._lock:
            self._workflows[workflow_id]["status"] = "ACTIVE"
    ...
    def is_active(self, workflow_id: str) -> bool:
        ...
        with self._lock:
            ...
            if entry is None:
                return False
            return entry["status"] in ("QUEUED", "ACTIVE")

    def active_count(self) -> int:
        """Return the number of currently active workflows."""
        with self._lock:
            return sum(
                1 for e in self._workflows.values()
                if e["status"] in ("QUEUED", "ACTIVE")
            )
